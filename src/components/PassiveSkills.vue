...
interface PassiveSkillDescriptions {
  forte_circuit: TalentSkill
  inherent_skill_1: TalentSkill
  inherent_skill_2: TalentSkill
}
...
<template>
  <div class="skill-container">
    <div class="forte-circuit-container">
      <div class="subskill-container">
        <h1>{{ PassiveSkill.forte_circuit.name }}</h1>
        <div v-for="(section, index) in PassiveSkill.forte_circuit.sections" :key="index">
          <h2>{{ section.title }}</h2>
    ...
    <div class="inherent-skills-container">
      <div class="subskill-container">
        <h1>{{ PassiveSkill.inherent_skill_1.name }}</h1>
        <div v-for="(section, index) in PassiveSkill.inherent_skill_1.sections" :key="index">
          <h2>{{ section.title }}</h2>
          <p v-html="highlightText(section.text, keywords)"></p>
          <ul>
            <li v-for="bullet in section.bullets" :key="bullet">
      ...
      <div class="subskill-container">
        <h1>{{ PassiveSkill.inherent_skill_2.name }}</h1>
        <div v-for="(section, index) in PassiveSkill.inherent_skill_2.sections" :key="index">
          <h2>{{ section.title }}</h2>
          <p v-html="highlightText(section.text, keywords)"></p>
          <ul>
            <li v-for="bullet in section.bullets" :key="bullet">
